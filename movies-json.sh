...
moviedir=/Some/Movies/ # directory where movies are stored.
file=movielist.txt
out=output.json
...
while read -r line; do # reads the file one line at a time.
	year="$( echo $line | grep -o --color=never -E '\S[0-9]{4}\S' | grep -o --color=never -E '[0-9]{4}')"  # gets the release year of the movie from the filename
